 ...
 */
exports.allQuestionsForGame = function(regionId="1", cb) {
    Question.find({official:true, numAnswers: {$lt : 3}, regionId: regionId.toString()}).select('-_id').exec(function(err, questions) {
        if (err) {
            console.log(err);
        } else {
            cb(questions);
