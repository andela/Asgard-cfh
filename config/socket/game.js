...
// check state for czar action
Game.prototype.beginGame = (self) => {
  if (self.state === 'czar pick black card') {
    self.stateChoosing(self);
  } else if (self.state === 'czar not found') {
    self.isCzarPick(self);
  }
};

// assign czar
Game.prototype.isCzarPick = (self) => {
  self.state = 'czar pick black card';
  self.table = [];
  if (self.czar >= self.players.length - 1) {
    self.czar = 0;
  } else {
